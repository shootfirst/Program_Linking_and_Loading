...
## 静态链接（以a.o和b.o作为例子）
  ...
  - 重定位表：
       ...
       * 介绍：对于可重定位的elf文件中必须包含重定位表，而对于每个需要重定位的段来说，都有对应的重定位表，用来描述如何修改该节中的内容。比如对于.text段来说，若需要重定位，则会有
               .rel.text的段保存.text段的重定位表。每个要被重定位的地方称为重定位入口。一个“重定位段(relocation section)”需要引用另外两个你：一个是符号表段，一个是被修改段。
               在符号表段中，段头的 sh_info 和 sh_link 成员分别指明了引用关系。
