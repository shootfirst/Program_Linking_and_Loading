...
## 各结构细节（以SimpleSection.c文件为例）
  ...
  - elf文件头
      ...
      * 字段解析：
          ...
          此字段指明段表(section header table)开始处在文件中的偏移量。如果没有段表，该值应设为 0。
          ...
          段表字符串表表项的索引。如果没有，此值应设置为 SHN_UNDEF。
      
          
    - 段表
    
        * 介绍：在目标文件中可以包含很多段(section)，所有这些段都登记在一张称为段表(section header table)的数组里。段表的每一个表项是一个 Elf32_Shdr结构，通过每一个表项可以定位到
                对应的段。一些表项的索引值被保留，有特殊的含义。ELF 文件的节头表中不会出现索引值为以下各值的表项，具体见elf官方文档。通常，目标文件中含有众多的段，段是文件中最大的部
		分，它们需要满足下列这些条件:
                ...
                + 在目标文件中，段与段之间可能会存在一些多余的字节，这些字节不属于任何段，比如为对齐而产生段与段之间的空隙。
            
                除了.text .data .bss这三个最常用的段之外，elf文件还包含其他段：
        ...
        * 数据结构：
         ...
         * 字段解析：
            ...
            本段的名字，整个名字的字符串并不存储在这里，它仅是一个索引号，值为.shstrtab 段表字符串表的偏移，那里存储了一个以’\0’结尾的字符串。
            
            + sh_type          
            
            本段的类型（不全）:
	    
                - SHT_NULL         0                   此值表明本段头是一个无效的（非活动的）段头，它也没有对应的段。本段头中的其它成员的值也都是没有意义的，默认第一个段是这个
                
                - SHT_PROGBITS     1                   此值表明本段所含有的信息是由程序定义的，本节内容的格式和含义都由程序来决定。数据段代码段都是这种类型
                
                - SHT_SYMTAB       2                   这两类段都含有符号表。目前，目标文件中最多只能各包含一个这两种段，但这种限制以后可能会取消。一般来说，SHT_SYMTAB 提供的
                                                       符号用于在创建目标文件的时候编辑链接，在运行期间也有可能会用于动态链接。SHT_SYMTAB 包含完整的符号表，它往往会包含很多在
                                                       运行期间(动态连接)用不到的符号。所以，一个目标文件可以再有一个 SHT_DYNSYM 段，它含有一个较小的符号表，专门用于动态连接
                                            
                - SHT_DYNSYM       11                  见上
                                                       
                - SHT_STRTAB       3                   此值表明本段是字符串表。目标文件中可以包含多个字符串表段
                
                - SHT_RELA         4                   此值表明本段是一个重定位段
                
                - SHT_HASH         5                   此值表明本段包含一张哈希表。所有参与动态连接的目标文件都必须要包含一个符号哈希表。目前，一个目标文件中最多只能有一个哈
                                                       希表，但这一限制以后可能会取消。
                                                       
                - SHT_DYNAMIC      6                   此值表明本段包含的是动态连接信息。目前，一个目标文件中最多只能有一个 SHT_DYNAMIC 段，但这一限制以后可能会取消
                
                - SHT_NOTE         7                   此值表明本段包含的信息用于以某种方式来标记本文件
                                                       
                - SHT_REL          9                   此值表明本段是一个重定位段，与上面区别在于数据结构中少一项加数
                
            - sh_flags         
            
            本段的一些属性（不全，下文所有属性未说明则都不全）：
	    
               + SHF_WRITE       0x1           本段所包含的内容在进程运行过程中是可写的
               
               + SHF_ALLOC       0x2           本段内容在进程运行过程中要占用内存单元。并不是所有段都会占用实际的内存，有一些起控制作用的段，在目标文件映射到进程空间时，并不需要
                                               占用内存
            ...
            若其可被加载，则表示在被加载后在进程地址空间中的虚拟地址，否则为0
            
            - sh_offset        8字节
            
            指明了本段所在的位置，该值是段的第一个字节在文件中的位置，即相对于文件开头的偏移量。单位是字节。如果该段的类型为 SHT_NOBITS 的话，表明这一段的内容是空的，段并不占用实
            际的空间，这时 sh_offset 只代表一个逻辑上的位置概念，并不代表实际的内容
            
            - sh_size          8字节
            
            指明段的大小，单位是字节。如果该段的类型为 SHT_NOBITS，此值仍然可能为非零，但没有实际的意义
            ...
            与链接相关的段才有意义（这两个字段可以和下面重定位表相关字段联合解析）：
            
                sh_type           sh_link                                           sh_info 
                SHT_DYNAMIC       该段使用的字符串表在段表中下标                       0 
                SHT_HASH          该段使用的符号表在段表中的下标                       0 
                SHT_REL/SHT_RELA  相应符号表在段表中的下标                            本重定位段所应用到目标段在段表中的索引值
                
                
            - sh_addralign     8字节
            
            此成员指明本段内容如何对齐字节，即该段的地址应该向多少个字节对齐。目前，sh_addralign 只能取 0、1或者 2 的正整数倍。如果该值为 0 或 1，表明本段没有字节对齐约束
            
            - sh_entsize       8字节
            
            有一些段的内容是一张表，其中每一个表项的大小是固定的，比如符号表。对于这种表来说，本成员指定其每一个表项的大小。如果此值为 0 则表明本段内容不是这种表格
    ...
    - 字符串表
        ...
        * 介绍：本节描述默认字符串表。字符串表中包含有若干个以’null’结尾的字符序列，即字符串。在目标文件中，这些字符串通常是符号的名字或者段的名字。在目标文件的其它部分中，当需要引
                用某个字符串时，只需要提供该字符串在字符串表中的序号即可。字符串表中的第一个字符串（序号为 0）永远是空串，即”null”，它可以用于表示一个空的名字或者没有名字。所以，字
                符串表的第一个字节是’null’。由于每一个字符串都是以’null’结尾，所以字符串表的最后一个字节也必然为’null’。字符串表也可以是空的，不含有任何字符串，这时，节头中的 
		sh_size成员必须是 0。一个目标文件中可能有多个字符串表，其中一个称为段字符串表(.shstrtab)，它包含所有段的名字。每一个段头的 sh_name 成员应该是一个索引值，它指向段字
		符串表中的一个位置，从这个位置开始到接下来第一个’null’字符为止的这个字符串，正是这个段的名字。既可以引用一个完整定义的字符串，即出现在两个’null’字符之间的整个串，也
		可以引用它的一部分。由于需要以’null’结尾，所以当引用一个子字符串时，一定是引用长串中靠后的那部分。
            
    - 符号表
        
        * 介绍：链接的本质就是将各个不同的目标文件“粘”在一起。在链接中目标文件之间相互拼合实际上就是目标文件之间对地址的引用。符号是指函数或者变量。我们可以把符号看成链接中的粘合剂。
                而符号表的作用就是对所有符号进行管理。这里面记录了目标文件中所有用到的符号。每个定义的符号有一个对应的值叫做符号值，一般对于变量和函数来说这个符号值就是它们的地址。
        ...
        * 数据结构：
         ...
         * 字段解析：
           ...
           任何一个符号表项的定义都与某一个段相联系，因为符号是为段而定义，在节中被引用。本数据成员即指明了相关联的段。本数据成员是一个索引值，它指向相关联的段在段表中的索引。在
	   重定位过程中，段的位置会改变，本数据成员的值也随之改变，继续指向段的新位置。当本数据成员指向下面三种特殊的段索引值时，本符号具有如下特别的意义：
           ...
           符号相对应的值，若不是common类型的，则表示该符号在段中的偏移，即在st_shndx指定的节中偏移量为st_value处。若是common类型的，则表示该符号对齐属性。而在可执行文件中，表示
           符号的虚拟地址
...
## 静态链接（以a.o和b.o作为例子）
  ...
  - 重定位表：
       ...
       * 介绍：这是干嘛的？链接器完成第一步合并后，它要修改重定位入口。怎么知道重定位入口的地方呢？就是通过重定位表。所有需要重定位的地方都被记录在重定位表中。对于可重定位的elf文件中
              必须包含重定位表，而对于每个需要重定位的段来说，都有对应的重定位表，用来描述如何修改该节中的内容。比如对于.text段来说，若需要重定位，则会有.rel.text的段保存.text段的
	      重定位表。每个要被重定位的地方称为重定位入口。一个“重定位段(relocation section)”需要引用另外两个段：一个是符号表段，一个是被修改段。在符号表段中，段头的 sh_info 和 
	      sh_link 成员分别指明了引用关系。
  ...
  第一步：扫描所有输入目标文件，获取各个段长度、属性和位置。并且将输入目标文件的符号表中所有的符号定义和符号引用收集起来。统一放到全局符号表中。这一步，链接器将能够获取的所有输入目
  标文件的段长度，并且将相似段合并。计算输出目标文件中各个段合并后的长度与位置，并建立映射关系。注意段是原来各个可重定位文件的，但是elf文件头、程序头表、段表是新的，而且相关字段会被修
  改，如段表中sh_addr和sh_offset，符号表中相关字段也会修改，称为符号地址的确定。原先符号的值是段中偏移量，现在变为虚拟地址。st_shndx也变为合并后相关段的下标，通过段表来查询该段首地
  址，从而计算出符号虚拟地址。重定位表相关字段也得修改，如r_info高24位是符号表的下标，此时变为全局符号表，故需要修改。
    
  第二步：符号解析与重定位：符号解析：首先链接器在重定位表中挨个修正。通过r_info的高24位找到相关符号在符号表中信息，首先如何找到相关符号表？不要忘了前面段头的sh_link，这个字段保存了
  符号表在段表中的下标。找到之后通过r_info的高24位在全局符号表中查询到表项a，通过a得到其虚拟地址。然后得找到重定位入口，如何寻找？r_offset此时只是偏移，要把他变成虚拟地址，如何计算？
  通过段头的sh_info得到需要重定位段的下标，获取其段头，得到其虚拟地址，从而计算得出重定位入口的虚拟地址。最后是指令修正方式，如何修正？通过r_info低8位来得到修正方法，使用S符号实际地
  址、A被修正位置的值和P被修正的位置，即重定位入口等数据修正。
  
  
  COMMON块存在的意义：
  
  当编译器将一个编译单元编译成目标文件后，若此编译单元包含了弱符号（未初始化的全局变量就是弱符号），那么此弱符号的大小是未知的，因为有可能在其他编译单元中，该符号所占的空间大于本编译
  单元，所以此时无法确定它在bss段的大小。但是在链接时，可以确定其大小，故链接时可以在bss段为其分配空间。
...
## 装载    
  ...
  装载的方式可以分为静态装入和动态装入。静态装入即一次性将程序运行所需的所有指令和数据全部装入内存，2022os内核采用的就是这个方案，此装载模式只适合程序所占空间较小的程序，所需内存数量
  小于物理内存数量。动态装入即把 程序最常用的部分驻留内存，而不常用的数据和指令存放于磁盘，这是得益于程序运行的局部性原理。
  
  覆盖装入和页映射是两个典型的动态装入方法，但现在几乎都是后者。页映射属于是虚拟存储机制的一部分。
  
  首先谈谈进程建立的过程，一般是分为三步：
  
    -  创建独立虚拟地址空间，实际就是创建root页表
    
    -  读取可执行文件头，建立虚拟空间与可执行文件的映射关系，程序执行时发生页错误时由trap处理相关代码为其分配物理页（若分配完则可以参考相关算法暂时回收物理页），这其实就是装载最关键
       的步骤，就是装载这一词的意义所在
    
    -  指令寄存器设置为可执行文件入口（实际较为复杂，涉及相关栈切换以及相关初始化，相关参数传递，不多介绍）
    
  
  最后谈谈linux装载elf文件大体过程：
  
  fork -> execve 
  
  execve首先查找被执行文件，找到后通过前128字节判断可执行文件类型，此例以elf为例，调用search_binary_handle()搜索对应可执行文件格式的装载过程，此函数调用load_script()函数，主要步
  骤：
  
  检查可执行文件格式的有效性，比如魔数，程序头表中段的数量
  ...
  将入口地址换为可执行文件的入口点，当syscall结束返回用户态时就开始执行可执行文件
